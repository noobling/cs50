...
{
    char p[] = "index.php";
    char* php = malloc(strlen(path) + strlen(p) + 1);
    
    strcpy(php, path);
    strcat(php, p); // create directory of path/index.php
    ...
    char h[] = "index.html";
    char* html = malloc(strlen(path) + strlen(h) + 1);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // iteratively get chars from file
...
const char* lookup(const char* path)
{
    ...
    if (strcasecmp(exten, ".css") == 0)  { return "text/css"; }
    if (strcasecmp(exten, ".html") == 0) { return "text/html"; }
    if (strcasecmp(exten, ".gif") == 0)  { return "image/gif"; }
    if (strcasecmp(exten, ".ico") == 0)  { return "image/x-icon"; }
    if (strcasecmp(exten, ".jpg") == 0)  { return "image/jpeg"; }
    if (strcasecmp(exten, ".js") == 0)   { return "text/javascript"; }
    if (strcasecmp(exten, ".php") == 0)  { return "text/x-php"; }
    if (strcasecmp(exten, ".png") == 0)  { return "image/png"; }
...
{
    char* dline = urldecode(line);
    
    if (strchr(dline, '"') != NULL)
    {
        error(400);
        return false;
    }
    if (dline[0] != 'G' || dline[1] != 'E' || dline[2] != 'T' || dline[3] != ' ')
    {
        error(405);
        return false;
    }
    
    if (strstr(dline, "HTTP/1.1") == NULL)
    {
        error(505);
        return false;
    }
    ...
    s = s + 1; // get rid of space char
    char* c = strchr(s, ' '); // search for next instance of space in dline
    if (c == NULL)
    {
        error(400);
        return false;
    ...
    // Get request-target and query from dline
    // c-s gives us the number of bytes for query and abs_path including '?'
    char* p = malloc((c-s+1));
    strncpy(p, s, c-s);
    p[c-s] = '\0'; // make last char null 
    
    // seperate query and path if query exists
    char* q = strchr(p, '?');
    if (q == NULL) // no query
    {
        strcpy(abs_path, p);
